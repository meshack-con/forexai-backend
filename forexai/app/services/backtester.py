...
class Backtester:
    def __init__(self, account_id: str, symbol: str, timeframe: str = "M15"):
    ...
    def _generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy().dropna()
        df["signal"] = 0
        df.loc[(df["ema9"] > df["ema21"]) & (df["ema9"].shift(1) <= df["ema21"].shift(1)) & (df["rsi"] < 70), "signal"] = 1
        df.loc[(df["ema9"] < df["ema21"]) & (df["ema9"].shift(1) >= df["ema21"].shift(1)) & (df["rsi"] > 30), "signal"] = -1
        return df

    def _simulate_trades(self, df: pd.DataFrame, stop_loss_pct: float = 0.5, take_profit_pct: float = 1.0) -> list[dict[str, Any]]:
        ...
        for timestamp, row in df.iterrows():
            ...
            if position is None and signal in (1, -1):
                ...
                stop_loss = entry_price * (1 - stop_loss_pct / 100) if position == "buy" else entry_price * (1 + stop_loss_pct / 100)
                take_profit = entry_price * (1 + take_profit_pct / 100) if position == "buy" else entry_price * (1 - take_profit_pct / 100)
                trades.append({
                    "symbol": self.symbol,
                    "mode": "backtest",
                    "type": position,
    ...
    def run(self, days: int = 180, stop_loss_pct: float = 0.5, take_profit_pct: float = 1.0) -> dict:
        ...
        report = BacktestReport(symbol=self.symbol, period_start=period_start, period_end=period_end, trades=trades)

        backtest_row = self._save_backtest(report)
        logger.info(f"Backtest completed for {self.symbol} with win_rate={report.win_rate}")
